...
// On utilise le système de précache de Workbox
// La variable self.__WB_MANIFEST correspondra automatiquement
// à nos fichiers statiques lors du build avec webpack (quand on fait npm run build)
// L'utilisation de la méthode precacheAndRoute permet d'automatiquement mettre
// ces fichiers dans le pré-cache.
...
registerRoute(
  ({request}) => {
    ...
    console.log(request.url.startsWith(BACKEND_URL + '/uploads'))
    return request.url.startsWith(BACKEND_URL + '/uploads')
  },
  new CacheFirst({
    cacheName: 'uploads'
  })
...
registerRoute(
  ({request}) => request.url === BACKEND_URL + '/',
  new NetworkFirst({
...
self.addEventListener('install', (event) => {
  ...
  // Ce code est commenté car géré par webpack
  // event.waitUntil(
